...
class TurbulentMixingWindow(QMainWindow):
    def __init__(self, model, parent=None):
        ...
        try:
            ...
            self.setStyleSheet("""
                ...
                QPushButton#start:pressed { background-color: #3d8b40; }
                QPushButton#start:disabled { background-color: #a5d6a7; }
                QPushButton#stop { background-color: #F44336; }
                QPushButton#stop:hover { background-color: #e53935; }
                QPushButton#stop:pressed { background-color: #d32f2f; }
                QPushButton#stop:disabled { background-color: #ef9a9a; }
            """)
            ...
            self.stop_button.setFont(QFont("Verdana", 10, QFont.Bold))
            ...
            self.ax1 = self.figure.add_subplot(121)  # Horizontal mixing with thermal front
            self.ax2 = self.figure.add_subplot(122)  # Vertical profile
            self.current_step = 0
            self.anim = None
    ...
    def update_plot(self, frame):
        ...
        try:
            ...
            # Update coupling object
            self.model.coupling.mixing_coeff = mixing_coeff
            self.model.coupling.wind_speed = wind_speed
            
            # Create thermal front in ocean_temps
            ny, nx = self.model.ocean_temps.shape
            x = np.arange(0, nx)
            y = np.arange(0, ny)
            X, Y = np.meshgrid(x, y)
            x0 = nx / 2 + 10 * np.sin(self.current_step * 0.01 * wind_speed)  # Dynamic front position
            L = 5.0  # Front width
            T_cold = 290
            T_hot = 300
            front = T_cold + (T_hot - T_cold) * 0.5 * (1 + np.tanh((X - x0) / L))
            ocean_temps = self.model.ocean_temps.copy() * 0.8 + front * 0.2  # Blend with model temps
            
            # Compute horizontal turbulent mixing across the front
            ...
            # Compute velocity gradients for vector field
            ...
            # Compute vertical mixing at front's center
            x_mid = int(nx / 2)
            self.temp_profile[0] = ocean_temps[ny//2, x_mid]  # Set surface temp to front's center
            Kv = mixing_coeff * np.exp(self.z / 50)  # Depth-dependent mixing (stronger at surface)
            ...
            dTdt[0] = 0  # Fix surface temp to front
            dTdt[-1] = dTdt[-2]
            ...
            # Plot horizontal mixing (thermal front with vectors)
            im = self.ax1.contourf(X, Y, ocean_temps, cmap="coolwarm", levels=20)
            self.ax1.quiver(X_sub, Y_sub, dx_sub, dy_sub, color="white", scale=20, scale_units="inches")
            ...
            # Plot vertical profile (Ekman spiral and temperature)
