...
class EddySimulationWidget(QWidget):
    def __init__(self, parent=None):
        ...
        self.density = None  # For baroclinic instability
        self.velocity_shear = None  # For barotropic instability
        self.buoyancy = None  # For mixed-layer instability
    ...
    def initialize_field(self, grid_size, eddy_strength, eddy_radius, num_eddies, refinement_threshold, 
        ...
        if self.enable_baroclinic:
            self.density = np.ones((grid_size, grid_size)) * 1025.0  # Base density (kg/m^3)
            self.density += baroclinic_shear * Y * stratification_param  # Linear stratification
        ...
        if self.enable_barotropic:
            ...
            self.velocity_shear += baroclinic_shear * Y  # Linear shear profile
        ...
        if self.enable_mli and grid_spacing_km < 10.0:
            ...
            self.buoyancy += buoyancy_gradient * Y  # Linear buoyancy gradient
            self.buoyancy += 0.01 * np.sin(2 * np.pi * X / 0.2)  # Submesoscale perturbations
        ...
        for _ in range(num_eddies):
            ...
            if self.enable_baroclinic:
                self.density += strength * 0.01 * np.exp(-r**2 / (2 * radius**2))  # Density perturbation
            if self.enable_barotropic:
                self.velocity_shear += strength * 0.05 * np.exp(-r**2 / (2 * radius**2))  # Shear perturbation
            if self.enable_mli and grid_spacing_km < 10.0:
                self.buoyancy += strength * 0.02 * np.exp(-r**2 / (2 * radius**2))  # Buoyancy perturbation
        ...
        for (x0, y0), strength, radius in zip(self.eddy_centers, self.eddy_strengths, self.eddy_radii):
            if abs(strength) > refinement_threshold:
                ...
                if self.enable_baroclinic:
                    ...
                    fine_grid_dict['density'] = fine_density
                
                if self.enable_barotropic:
                    fine_shear = strength * 0.05 * np.exp(-r**2 / (2 * radius**2))
                    fine_grid_dict['velocity_shear'] = fine_shear
                
                if self.enable_mli and grid_spacing_km < 10.0:
                    fine_buoyancy = strength * 0.02 * np.exp(-r**2 / (2 * radius**2))
                    fine_grid_dict['buoyancy'] = fine_buoyancy
    ...
    def compute_mli_effects(self, buoyancy, dx, mixed_layer_depth):
        ...
        # Vorticity tendency from lateral buoyancy gradients
        ...
        vorticity_tendency = 0.1 * mixed_layer_depth * (grad_b_x - grad_b_y)  # Scaled by mixed-layer depth
        return vorticity_tendency
    ...
    def update_simulation(self, vorticity_diffusion, rotation_rate, background_flow, decay_rate, 
        ...
        # Add diffusion, strain, and instabilities
        ...
        # Apply baroclinic, barotropic, and mixed-layer instability effects
        if self.enable_baroclinic:
        ...
        for fine_grid_info in self.fine_grids:
            ...
            if i_start >= 0 and i_end < self.grid_size and j_start >= 0 and j_end < self.grid_size:
                ...
                if self.enable_baroclinic:
                    ...
                    self.density[j_start:j_end, i_start:i_end] = coarse_density[:j_end-j_start, :i_end-i_start]
                if self.enable_barotropic:
                    coarse_shear = fine_grid_info['velocity_shear'][::self.refinement_factor, ::self.refinement_factor]
                    self.velocity_shear[j_start:j_end, i_start:i_end] = coarse_shear[:j_end-j_start, :i_end-i_start]
                if self.enable_mli and grid_spacing_km < 10.0:
                    coarse_buoyancy = fine_grid_info['buoyancy'][::self.refinement_factor, ::self.refinement_factor]
                    self.buoyancy[j_start:j_end, i_start:i_end] = coarse_buoyancy[:j_end-j_start, :i_end-i_start]
            ...
            for i in range(self.grid_size):
                for j in range(self.grid_size):
                    ...
                    if self.enable_barotropic:
                        ...
                        r = int(r * (1 + 0.2 * shear))
                    if self.enable_mli and self.grid_spacing_km < 10.0:
                        b_mli = self.buoyancy[i, j] / 0.1  # Normalize buoyancy
                        b = int(b * (1 + 0.3 * b_mli))  # Blue for buoyancy
...
class OceanicEddyAndFrontWindow(QDialog):
    def __init__(self, parent=None):
        ...
        try:
            ...
            self.setGeometry(200, 200, 600, 950)
    ...
    def initialize_simulation(self):
        try:
            ...
            logging.debug("Eddy simulation initialized with ageostrophic, non-hydrostatic, baroclinic, barotropic, and mixed-layer instability dynamics")
    ...
    def update_simulation(self):
        try:
            ...
            self.simulation_widget.update_simulation(vorticity_diffusion, rotation_rate, background_flow, 
                                                   ...
                                                   mixed_layer_depth, buoyancy_gradient)
        except ValueError as e:
