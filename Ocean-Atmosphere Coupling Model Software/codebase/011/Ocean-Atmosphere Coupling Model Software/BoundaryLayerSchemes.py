...
class BoundaryLayerSchemesWindow(QMainWindow):
    ...
    def update_plot(self, frame):
        ...
        try:
            ...
            temp_diff = ocean_temps - atm_temps
            self.time_steps.append(self.current_step * self.dt)
            self.bulk_fluxes.append(bulk_heat_flux.flatten())
            self.kpp_fluxes.append(kpp_heat_flux.flatten())
            self.temp_gradients.append(temp_diff.flatten())
            ...
            y = np.arange(self.ny)
            if scheme == "Bulk Scheme":
                data = self.bulk_heat_flux
                title = f"Bulk Heat Flux (W/m²)\nat t={self.current_step * self.dt:.2f}s"
                cmap = "coolwarm"
                label = "Heat Flux (W/m²)"
            else:
                data = self.kpp_diffusivity
                title = f"KPP Diffusivity (m²/s)\nat t={self.current_step * self.dt:.2f}s"
                cmap = "viridis"
            ...
            # Add annotations (values) to heatmap
            for i in range(0, self.ny, max(1, self.ny // 5)):  # Sparse annotations for readability
            ...
            # Scatter plot: Flux/Diffusivity vs. Temperature Gradient
            if scheme == "Bulk Scheme":
                values = self.bulk_fluxes[-1]
            else:
                values = self.kpp_diffusivity.flatten()
            
            scatter = self.ax2.scatter(self.temp_gradients[-1], values, c=self.time_steps[-1] * np.ones_like(values), cmap="plasma", alpha=0.6)
            self.ax2.set_title("Flux/Diffusivity vs. Temp Gradient")
            self.ax2.set_xlabel("T_ocean - T_atm (K)")
            self.ax2.set_ylabel(label)
            self.ax2.grid(True, color="gray", linestyle="--", alpha=0.5)
            if not hasattr(self.ax2, 'colorbar'):
                self.figure.colorbar(scatter, ax=self.ax2, label="Time (s)")
