...
class TwoWayCoupling:
    def __init__(self, drag_coeff, wind_speed, precip_rate, evap_rate, solar_forcing, 
                 longwave_coeff, mixing_coeff, co2_transfer_coeff):
    ...
    def compute_freshwater_flux(self, salinity, moisture):
        logging.debug("Computing freshwater flux")
        try:
            precip = self.precip_rate * (1 + 0.5 * np.clip(moisture / 0.01, 0, 2))
            F = precip - self.evap_rate
            dS_dt = -salinity * F / self.rho_water
            return np.clip(dS_dt, -1e-3, 1e-3), precip
        except Exception as e:
    ...
    def compute_co2_flux(self, co2_ocean, co2_atm):
        logging.debug("Computing CO2 flux")
        try:
            pCO2_ocean = co2_ocean / self.co2_solubility
            pCO2_atm = co2_atm
            F_co2 = self.co2_transfer_coeff * (pCO2_ocean - pCO2_atm)
