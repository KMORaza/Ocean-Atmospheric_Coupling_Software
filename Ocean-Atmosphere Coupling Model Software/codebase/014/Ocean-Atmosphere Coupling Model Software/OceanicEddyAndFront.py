...
class EddySimulationWidget(QWidget):
    ...
    def initialize_field(self, grid_size, eddy_strength, eddy_radius, num_eddies, refinement_threshold, 
        ...
        if self.enable_baroclinic:
            ...
            self.density = np.clip(self.density, 1020.0, 1030.0)  # Prevent extreme densities values
        ...
        if self.enable_barotropic:
            ...
            self.velocity_shear = np.clip(self.velocity_shear, -0.1, 0.1)  # Prevent extreme shear
        ...
        if self.enable_mli and grid_spacing_km < 10.0:
            ...
            self.buoyancy = np.clip(self.buoyancy, -0.1, 0.1)  # Prevent extreme buoyancy
    ...
    def compute_baroclinic_effects(self, density, dx):
        ...
        vorticity_tendency = np.clip(vorticity_tendency, -0.1, 0.1)  # Prevent numerical instability
    ...
    def update_simulation(self, vorticity_diffusion, rotation_rate, background_flow, decay_rate, 
        ...
        if dt * max_velocity / dx > 0.5:
            dt = 0.5 * dx / max_velocity  # Adjust timestep for stability
        self.time += dt
        ...
        self.vorticity = np.clip(self.vorticity, -10.0, 10.0)  # Prevent numerical blow-up
